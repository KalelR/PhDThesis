...
As just seen, stable equilibria are the only possible attractors in linear systems. This is far from true in the case of nonlinear systems. The possible long-term dynamics then can be much more interesting. Attractors can be extended structures in state space, including more than a single point. Figure XX shows examples of other types of attractors. These include a closed curve, wherein trajectories revolve around, and repeat after some time $T>0$. If this closed curve attracts trajectories, it is called a limit cycle. Another type of attractor is a torus. XX. Also Chaos. 
